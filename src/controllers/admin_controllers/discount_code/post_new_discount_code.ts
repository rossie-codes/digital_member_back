...
async function postNewDiscountCode(c: Context): Promise<Response> {
  try {
    ...
    if (
      ...
      minimum_spending < 0 ||
      !["single_use", "once_per_customer", "unlimited"].includes(
        use_limit_type
      ) ||
    ...
    if (isNaN(validFromDate.getTime()) || isNaN(validUntilDate.getTime())) {
      throw new HTTPException(400, {
        message: "Invalid date format for valid_from or valid_until",
      });
    }
    ...
    let discount_code_status: "active" | "expired" | "suspended" | "scheduled" =
      "suspended";
    ...
    if (discount_type === "fixed_amount") {
      if (typeof discount_amount !== "number" || discount_amount <= 0) {
        throw new HTTPException(400, {
          message:
            "Invalid or missing discount_amount for fixed_amount discount type",
        });
      }
      ...
      if (
        ...
        throw new HTTPException(400, {
          message:
            "Invalid or missing discount_percentage for percentage discount type",
        });
      }
    ...
    function decodeShopifyId(globalId: string): string {
      ...
      const match = base64Decoded.match(
    ...
    try {
      ...
      if (
        error.code === "23505" &&
        error.constraint === "discount_code_discount_code_key"
      ) {
